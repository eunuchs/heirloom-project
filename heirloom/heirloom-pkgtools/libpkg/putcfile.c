...
/*	from OpenSolaris "putcfile.c	1.12	06/02/27 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)putcfile.c	1.3 (gritter) 2/25/07
 */
...
putcfile(struct cfent *a_ept, FILE *a_fp)
{
	...
	if ((a_ept->ftype == 'c') || (a_ept->ftype == 'b')) {
		if (a_ept->ainfo.major == BADMAJOR) {
			...
			if (fprintf(a_fp, " %d", (int)a_ept->ainfo.major) < 0)
				return (-1);
		}
		...
		if (a_ept->ainfo.minor == BADMINOR) {
			...
			if (fprintf(a_fp, " %d", (int)a_ept->ainfo.minor) < 0)
				return (-1);
		}
	}
...
putcvfpfile(struct cfent *a_ept, VFP_T *a_vfp)
{
	...
	if ((a_ept->ftype == 'c') || (a_ept->ftype == 'b')) {
		...
		if (a_ept->ainfo.major == BADMAJOR) {
			...
			vfpPutc(a_vfp, ' ');
			vfpPutInteger(a_vfp, (int)a_ept->ainfo.major);
		}
		...
		if (a_ept->ainfo.minor == BADMINOR) {
			...
			vfpPutc(a_vfp, ' ');
			vfpPutInteger(a_vfp, (int)a_ept->ainfo.minor);
		}
	}
