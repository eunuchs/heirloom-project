...
/*	from OpenSolaris "pkgweb.c	1.6	06/03/15 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)pkgweb.c	1.7 (gritter) 2/26/07
 */
...
static boolean_t get_ENV_proxyport(PKG_ERR *, unsigned short *);
...
static boolean_t parse_url_proxy(PKG_ERR *, char *, char *, unsigned short);
 ...
 */
boolean_t
get_proxy_port(PKG_ERR *err, char **proxy, unsigned short *port)
{
	if (*proxy != NULL) {
		if (!path_valid(*proxy)) {
			/* bad proxy supplied */
...
web_session_control(PKG_ERR *err, char *url, char *dwnld_dir,
    keystore_handle_t keystore, char *proxy, unsigned short proxy_port,
 ...
 */
static boolean_t
parse_url_proxy(PKG_ERR *err, char *url, char *proxy, unsigned short proxy_port)
{
	boolean_t ret = B_TRUE;
	if (!path_valid(url)) {
		ret = B_FALSE;
 ...
 */
static boolean_t
get_ENV_proxyport(PKG_ERR *err, unsigned short *port)
{
	char *buf;
